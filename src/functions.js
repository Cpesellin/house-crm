...
// ══════════════════════════════════════════════════════════════════
// 30. CIERRES + COMISIONES FLEXIBLES (N participantes, % libres)
// ══════════════════════════════════════════════════════════════════

const BONO_FASE_A = 50000;
const _PCTS = { venta: 0.03, arriendo: 0.10 };
const _PCT_OPTS = { venta: [2,3,4,5], arriendo: [5,8,10] };
const _ROL_LABELS = { house:'🏢 Inmobiliaria House', comisionista_inmueble:'🤝 Comisionista (inmueble)', comisionista_comprador:'🤝 Comisionista (comprador)', asesor_captador:'👤 Asesor captador', referidor:'🎁 Referidor', otro:'📋 Otro' };
...
window.abrirFormularioCierre = function(inmId, estadoDestino) {
  ...
  if (iDiv) { try { JSON.parse(iDiv.textContent || '[]').filter(i => i.score === 'verde' && i.estado !== 'descartado').forEach(i => { interesesOpts += '<option value="' + i.id + '" data-nombre="' + (i.usuario?.nombre || '') + '">' + (i.usuario?.nombre || 'Sin nombre') + '</option>'; }); } catch(e) {} }
...
window._cierreFillNombre = function(sel) { const n = sel.options[sel.selectedIndex]?.dataset?.nombre || ''; const inp = document.getElementById('cierre-contraparte'); if (inp && n) inp.value = n; };

window._cierreRenderPctBar = function(tipo) {
  const bar = document.getElementById('cierre-pct-bar'); if (!bar) return;
...
window._cierreRenderParticipantes = function(tipo) {
  const el = document.getElementById('cierre-participantes'); if (!el) return;
...
window._cierreApplyPreset = function(idx) {
  const preset = _PRESETS[idx]; if (!preset) return;
  const parts = window._cierreParticipantes || [];
  // Adjust number of participants to match preset
...
window._cierreRefreshPreview = function(tipo) {
  const raw = (document.getElementById('cierre-precio')?.value || '').replace(/[^0-9]/g, '');
  const precio = parseInt(raw) || 0;
  const div = document.getElementById('cierre-preview'); if (!div) return;
...
window._guardarCierre = async function(inmId, tipo) {
  ...
  let cierreId;
  try {
    const row = { inmueble_id: inmId, tipo, precio_final: precio, fecha_cierre: fecha, contraparte_nombre: contraparte, duracion_meses: duracion, interes_id: interesId || null, nota, comision_total: total, comision_captador: captMonto, comision_casa: casaMonto, comision_porcentaje: pct, captador_id: p?.captador_id || u.id, cerrado_por: u.id };
    const { data: cierre, error } = await SB().from('cierres').insert(row).select('id').single();
    if (error) { if (/cierres/i.test(error.message) && /does not exist/i.test(error.message)) { window.toast('⚠️ Falta correr sql/23-cierres.sql', 'terr'); return; } throw error; }
    cierreId = cierre.id;
  } catch (e) { console.error('[_guardarCierre]', e); window.toast('Error: ' + e.message, 'terr'); return; }

  // 2. Insert participantes
  try {
    const rows = parts.map(pt => ({ cierre_id: cierreId, usuario_id: pt.usuario_id || null, nombre_externo: pt.nombre || null, rol_comision: pt.rol, porcentaje: pt.porcentaje, monto: Math.round(total * pt.porcentaje / 100) }));
  ...
  if (interesId) await SB().from('intereses_compradores').update({ estado: 'cerrado', updated_at: new Date().toISOString() }).eq('id', interesId);
  if (estado === 'Arrendado' && typeof window.registrarComisionArrendado === 'function') { try { await window.registrarComisionArrendado(inmId); } catch(e) {} }
...
// Backward compat: old marcarPagoCierre still works for old cierres
window.marcarPagoCierre = async function(cierreId, fase) {
  const campo = fase === 'a' ? 'fase_a_pagada' : fase === 'b' ? 'fase_b_pagada' : 'pagada';
  const ok = await window.cfShow('💵', '¿Marcar pago?', ''); if (!ok) return;
  await SB().from('cierres').update({ [campo]: true, [campo + '_at']: new Date().toISOString() }).eq('id', cierreId);
  window.toast('💵 Pago registrado');
  if (typeof window.rMisNegocios === 'function') window.rMisNegocios();
};
...
// ══════════════════════════════════════════════════════════════════
// 32. GESTIÓN DE ROLES — Upgrade/Downgrade/Crear usuario
// ══════════════════════════════════════════════════════════════════
...
function _renderConfirmCambioRol() {
...
window.cambiarRolUsuario = async function(userId, nuevoRolKey) {
  ...
  // History
  try {
    await SB().from('historial_roles_usuario').insert({ usuario_id: userId, rol_anterior: curRolKey, rol_nuevo: nuevoRolKey, tipo_anterior: usr.tipo_usuario, tipo_nuevo: upd.tipo_usuario, direccion: dir, cambiado_por: U().id, motivo });
...
window.abrirCrearUsuario = function() {
  ...
  _renderCrearUsuarioModal();
};

function _renderCrearUsuarioModal() {
  ...
  if (step === 1) {
    ...
    [{l:'Nombre completo *',id:'cu_nombre',ph:'Ej: Carlos Mejía'},{l:'Correo electrónico *',id:'cu_email',ph:'Ej: carlos@house.com',t:'email'},{l:'Teléfono (WhatsApp) *',id:'cu_tel',ph:'Ej: 310 555 1234',t:'tel'}].forEach(f => {
...
async function _crearUsuarioFinal(rolKey) {
  ...
  try {
    ...
    if (error) { if (/creado_por|estado_usuario|notas_admin/i.test(error.message)) { delete upd.creado_por; await SB().from('usuarios').insert(upd); } else throw error; }
    document.getElementById('crearUsrDlg')?.remove();
