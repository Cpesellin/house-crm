...
window.showPublicView = async function(id) {
  ...
  try {
    ...
    if (!p) { app.innerHTML = '<div class="emp"><span class="emp-i">🏠</span><h3>Inmueble no encontrado</h3></div>'; return; }
    ...
    if (fotos.length > 0) {
      ...
      h += `<span class="gal-count" id="pub-ct">1/${fotos.length}</span></div>`;
    }
    h += `<div class="pub-info"><div class="pub-tipo">${emo(p.tipo)} ${p.tipo || 'Inmueble'}</div><div class="pub-ciudad">📍 ${p.direccion_publica ? p.direccion_publica + ' · ' : ''}${p.ciudad || ''}</div>`;
    if (p.precio_venta > 0) h += `<div class="pub-precio">${fm(p.precio_venta)}</div>`;
    if (p.precio_arriendo > 0) h += `<div class="pub-arriendo">${fm(p.precio_arriendo)}/mes</div>`;
    const specs = [];
    if (p.habitaciones) specs.push({ v: p.habitaciones, l: 'Hab.', i: '🛏️' });
    if (p.banos) specs.push({ v: p.banos, l: 'Baños', i: '🚿' });
    if (p.area_construida) specs.push({ v: p.area_construida + 'm²', l: 'Construida', i: '📐' });
    if (specs.length) h += `<div class="pub-specs">${specs.map(s => `<div class="pub-spec"><div class="pub-spec-v">${s.i} ${s.v}</div><div class="pub-spec-l">${s.l}</div></div>`).join('')}</div>`;
    if (p.descripcion_cliente) h += `<div style="margin-top:14px;padding-top:14px;border-top:1px solid var(--g100)"><div style="font-size:13px;line-height:1.5">${p.descripcion_cliente}</div></div>`;
    h += `<div class="pub-cta"><a class="wa" href="https://wa.me/${capTel}?text=${encodeURIComponent('Hola ' + capNom + ', me interesa el ' + p.tipo + ' en ' + p.ciudad)}" target="_blank">💬 WhatsApp</a><a class="tel" href="tel:+${capTel}">📞 Llamar</a></div>`;
    h += '</div></div><div class="pub-footer">Inmobiliaria House · Pereira</div></div>';
    app.innerHTML = h;
    window._pubFotos = fotos.map(f => f.url); window._pubIdx = 0;
  } catch (e) { app.innerHTML = '<div class="emp"><span class="emp-i">❌</span><h3>Error</h3><p>' + e.message + '</p></div>'; }
};

window.pubNav = function(dir) { const f = window._pubFotos; if (!f || !f.length) return; window._pubIdx = (window._pubIdx + dir + f.length) % f.length; window.pubGo(window._pubIdx); };
window.pubGo = function(i) { const f = window._pubFotos; if (!f) return; window._pubIdx = i; const img = document.getElementById('pub-img'); if (img) { img.src = f[i]; } const ct = document.getElementById('pub-ct'); if (ct) ct.textContent = (i + 1) + '/' + f.length; };
