  ...
  ls.slice(0, 60).forEach(p => {
    ...
    if (_isExt) {
      ...
      if (_isVisitor) {
        ...
        // Cliente: priorizar Ver detalle (acceso a descripcion_cliente vía enlace público) + Me interesa estructurado
    ...
    // Share button — DISPONIBLE PARA TODOS los perfiles (visitante, público,
    // interno). Posicionado a la izquierda del corazón.
    const _shareCode = (p.codigo_house || p.id || '').replace(/'/g, '');
    const _shareTitle = ((p.tipo || 'Inmueble') + ' en ' + (p.barrio || p.ciudad || '')).replace(/'/g, '');
    const shareBtn = `<button aria-label="Compartir" style="position:absolute;top:8px;right:48px;z-index:2;width:32px;height:32px;min-width:32px;border-radius:50%;background:rgba(0,0,0,.45);border:none;color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(4px)" onclick="event.stopPropagation();window.shareInmueble&&window.shareInmueble('${_shareCode}','${_shareTitle}')"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="12" r="2"/><circle cx="18" cy="6" r="2"/><circle cx="18" cy="18" r="2"/><path d="M8 11l8-4"/><path d="M8 13l8 4"/></svg></button>`;
